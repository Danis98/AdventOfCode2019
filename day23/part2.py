...
def nic_read(id):
    global packet_queue, initialized, idle, NAT, last_NAT, end
...
def nic_out(id, o):
    global vm_buffer, packet_queue, NAT
    vm_buffer[id].append(o)
    if len(vm_buffer[id]) == 3:
        if vm_buffer[id][0] == 255:
            NAT = (vm_buffer[id][1], vm_buffer[id][2])
        else:
