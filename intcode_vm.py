tape = dict([(i, int(x)) for (i, x) in enumerate(open("dayXX.input", "r").read().replace("\n", "").strip().split(","))])
...
vms = [None for i in range(THREAD_NUM)]
end = False
...
vm_ctr = 0
while not end:
    id = vm_ctr % THREAD_NUM
    vms[id].exec = True
    vms[id].run(lambda : stdin(id), lambda o: stdout(id, o))
    vm_ctr += 1
